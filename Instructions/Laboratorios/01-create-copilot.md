...
lab:
  ...
  description: En este ejercicio, usarás Copilot Studio para crear un agente sencillo que responda preguntas de empleados sobre directivas de gastos en una corporación ficticia.
  duration: 10 minutos
  level: 100
  islab: true
---
...
En este ejercicio, usarás Copilot Studio para crear un agente sencillo que responda preguntas de empleados sobre directivas de gastos en una corporación ficticia.
...
1. Escribe el siguiente prompt en **Start building by describing what your agent needs to do**:
    ...
    ```prompt
    Crea un agente para ayudar a los empleados con reclamaciones de gastos.
    ```
...
1. Agrega `Evita proporcionar asesoramiento fiscal.` a las **Instructions**.
...
1. Ahora prueba el siguiente prompt:
    ...
    ```prompt
    ¿Con quién debo comunicarme para enviar una reclamación de gastos?
    ```
...
1. Probemos otro prompt:
    ...
    ```prompt
    ¿Cuál es el límite de gastos para una estancia en hotel?
    ```
    ...
    Nuevamente, la respuesta puede ser adecuada pero genérica. En una organización real, querrías que el agente proporcionara una respuesta más específica basada en las directivas de gastos de la empresa.
...
1. En el cuadro de diálogo **Add from description with Copilot**, asigna al nuevo tema el nombre `Preguntar por contacto de gastos` y escribe el siguiente texto para indicar a Copilot Studio qué debe hacer el tema:

    ```prompt
    Cuando el usuario pregunte con quién comunicarse sobre reclamaciones de gastos, dile que envíe un correo electrónico a finance@contoso.com
    ```
...
1. Después de una breve espera, se debería crear un nuevo tema llamado *Preguntar por contacto de gastos* y abrirse en el lienzo de creación, donde debería verse similar a esto:

    ![Screenshot of the copilot-generated topic](media/copilot-studio-topic.png)

    El nuevo tema debe desencadenarse mediante frases que pregunten por un contacto para gastos y responder con un mensaje que indique al usuario que envíe un correo electrónico a la dirección correspondiente.
...
1. Abre una nueva pestaña del navegador y ve a `https://github.com/MicrosoftLearning/mslearn-copilotstudio/raw/main/expenses/Expenses_Policy.docx` para descargar localmente el [documento de directiva de gastos](https://raw.githubusercontent.com/MicrosoftLearning/mslearn-copilotstudio/main/expenses/Expenses_Policy.docx). Este documento contiene detalles de la directiva de gastos de la corporación ficticia Contoso.
...
1. En la sección **Upload file**, usa **select to browse** para cargar el documento de directiva de gastos que descargaste anteriormente y selecciona **Add to agent**.
...
1. En el panel **Demo website**, especifica la siguiente configuración:
    - **Welcome message**: `Pregúntame sobre reclamaciones de gastos`
    - **Conversation starters**:

        ```prompt
        "Hola"
        "¿Con quién debo comunicarme para consultas sobre gastos?"
        "¿Cuáles son los límites de gastos para vuelos?"
        ```
...
1. Intenta hacer algunas preguntas de seguimiento, como:
    - `¿Y los vuelos?`
    - `¿Qué directrices hay para gastos de entretenimiento?`
    - `¿Cuáles son los límites de gastos para comidas?`

1. Prueba algunas preguntas más y revisa las respuestas de tu agente. Tendrá funcionalidad limitada, pero debería poder proporcionar respuestas relevantes a preguntas sobre reclamaciones de gastos.
